...
from consts import TASK_TO_SENTENCE_KEY, TASK_TO_VAL_SPLIT_NAME
...
class TokenizedDataset(Dataset):
    def __init__(self, bert_path: str, task: str, split: str):
        ...
        dataset = load_dataset(bert_path, task, split=split)
...
def get_data_loaders(config: Config) -> Tuple[DataLoader, DataLoader]:
    train_set = TokenizedDataset(bert_path=config.data_path, task=config.task, split="train")
    val_set = TokenizedDataset(
        bert_path=config.data_path,
        task=config.task,
