...
from transformer.modeling_bert_quant_bit import BertForSequenceClassification
...
from models.ee.simple import SimpleEarlyExitModel
from models.ee.ztw import ZTWModel

sns.set_style("darkgrid")


class EarlyExitModel(pl.LightningModule):
    def __init__(self, config: Config, model_name):
        super().__init__()
        ...
        self.bert_0 = BertForSequenceClassification.from_pretrained(config.model_path, model_name[0], from_tf=False)
        self.bert_1 = BertForSequenceClassification.from_pretrained(config.model_path, model_name[1], from_tf=False)
        self.bert_0.to(self.config.device)
        self.bert_1.to(self.config.device)
        self.bert_0.eval()
        self.bert_1.eval()
    ...
    def _prepare_batch(self, batch: Any) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        with torch.no_grad():
            # print('_prepare_batch: ',batch)
            keys = ["input_ids", "attention_mask", "token_type_ids", "labels", "seq_length"]
            ...
            atts={0:[],1:[]}
            reps={0:[],1:[]}
            logits[0], atts[0], reps[0] = self.bert_0(
                **{key: value.to(self.config.device) for key, value in batch.items()},
                output_hidden=True,
            )
            logits[1], atts[1], reps[1] = self.bert_1(
                **{key: value.to(self.config.device) for key, value in batch.items()},
                output_hidden=True,
            )
            # print(reps)
            # reps = (reps[0] + reps[1]) /2
            
            reps = [reps[0] + reps[1] for reps[0], reps[1] in zip(reps[0], reps[1])]
            states = torch.stack(reps[1:], dim=1)
            logits = (logits[0] + logits[1]) /2
        return logits, states, labels
    
    def _epoch_end(self, outputs: List[Dict[str, torch.Tensor]], key: str) -> None:
        task =  compute_modes[self.config.task]
        head_f1s = {i + 1: torchmetrics.F1Score(task) for i in range(self.num_hidden_layers)}
        head_accs = {i + 1: torchmetrics.Accuracy(task) for i in range(self.num_hidden_layers)}
    ...
    def _log_average_layer_vs_accuracy(
        ...
        for thresh in tqdm(
            ...
            accuracy = torchmetrics.Accuracy(task)
    ...
    def _log_accuracy_with_forced_exits_at_layers(
        ...
        for layer_idx in tqdm(
            ...
            layer_accuracy = torchmetrics.Accuracy(task)
            for batch in dataloader:
                _, states, labels = self._prepare_batch(batch)
